...
""")
    ...
    def get_students_with_grade_lower_than_5_sorted(self):
        try:
            ...
            subject = self.__subject_service.find_by_id(subject_id)
            print(subject)
            avg_grades = self.__grade_service.stat_1_rec(subject)
            for avg_grade in avg_grades:
                print(f'Average grade: {avg_grade[0]}, {avg_grade[1]}')
        except ValueError as e:
            print(e)
    ...
    def add_random_subject(self):
        try:
            n = int(input('Nr. of subjects: '))
            subjects = self.__subject_service.add_random_subject(n)
            for x in subjects:
                print(x)
        except ValueError as e:
            print(e)
