...
from sort import selection_sorted, cocktail_sorted
...
def test_sort(fn):
    arr = [4, 5, 0, 1, 100, 300, 30000, 99999999999999]
    assert fn(arr) == sorted(arr)
    assert fn(arr, reverse=True) == sorted(arr, reverse=True)
    assert fn(arr, key=lambda x: -x) == sorted(arr, key=lambda x: -x)
...
def run_tests():
    ...
    test_sort(selection_sorted)
    test_sort(cocktail_sorted)
