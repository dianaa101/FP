...
class SubjectService:
    ...
    def add_random_subject(self, n):
        subjects = []

        for x in range(n):
            subject_name = random_name_between(4, 10)
            teacher_first_name = random_name_between(2, 7)
            teacher_last_name = random_name_between(4, 9)
            id_ = random.randint(0, 10000)
            id_ = self.__repo.get_available_id(preferred_id=id_)

            subject = Subject(id_, f' {subject_name}', f'{teacher_first_name}{teacher_last_name}')
            self.__validator.validate_subject(subject)
            self.__repo.add(subject)
